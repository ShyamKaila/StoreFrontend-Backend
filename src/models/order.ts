...
export class OrderStores {

    async create(or: Orders): Promise<Orders> {
        try {
      ...
      const result = await conn
          .query(sql, [or.status, or.user_id])
      ...
      return order
            ...
            throw new Error(`Could not add new order ${or.status} ${or.user_id}. Error: ${err}`)
        }
    }
